class Api::V1::User::SignersController < Api::V1::User::ApplicationController
  before_action :set_signer
  skip_before_action :validate_token, only: [:document, :sign]
  ...
  def sign
    if @signer.update(status: "signed")
